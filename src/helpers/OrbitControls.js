 ...
 */
import {Vector3, Vector2, Quaternion, Box3, Sphere, Spherical, EventDispatcher, MOUSE} from 'three';
...
const OrbitControls = function ( object, domElement ) {
	...
	this.clampInstance = (instance, light = null) => {
		...
		// this.object.far = sphere.radius * 2.5;
		...
		//light.shadow.camera.position.set(box.max.x, box.max.y, box.max.z);
		light.castShadow = true;

		//console.log(light)
	};

	this.clampLight = (instance, light) => {
		let box = new Box3().setFromObject(instance);
		let sphere = new Sphere();
		box.getBoundingSphere(sphere);
		...
		light.position.copy(box.max.clone().normalize()).setY(box.max.y);
		light.shadow.camera.position.set(box.max.x, box.max.y, box.max.z);
		light.castShadow = true;
	};
